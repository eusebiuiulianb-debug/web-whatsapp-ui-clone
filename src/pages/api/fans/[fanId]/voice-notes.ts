...
export default async function handler(req: NextApiRequest, res: NextApiResponse<VoiceNoteResponse>) {
  ...
  const fanIdParam = typeof req.query.fanId === "string" ? req.query.fanId.trim() : "";
  if (!fanIdParam || !isSafeChatId(fanIdParam)) {
  ...
  try {
    ...
    const messageId = `${fanIdParam}-${Date.now()}`;
    const extension = getExtensionFromMime(mimeType);
    const relativeDir = path.posix.join("uploads", "voice-notes", fanIdParam);
    const destDir = path.join(process.cwd(), "public", "uploads", "voice-notes", fanIdParam);
    ...
    console.error("fan voice-note upload error", err);
