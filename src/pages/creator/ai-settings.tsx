            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="flex flex-col gap-2">
                  ...
                  <select
                    value={form.tone}
                    onChange={(e) => setForm((prev) => (prev ? { ...prev, tone: e.target.value } : prev))}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="flex items-center gap-3 border border-slate-800 rounded-xl px-4 py-3 bg-slate-900/70">
                  ...
                  <div className="flex flex-col">
                    ...
                    <p className="text-xs text-slate-400">
                      Activa si quieres que la IA conteste en tu nombre cuando la cola esté despejada.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="text-xs text-slate-300">
                Usados hoy:{" "}
                {status
                  ? `${status.usedToday}/${status.hardLimitPerDay ?? "∞"}`
                  : "—"}{" "}
                · Créditos disponibles: {status ? status.creditsAvailable : "—"}
              </div>
        ...
        <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-4 sm:p-6">
          <div className="flex items-center justify-between mb-3">
            <div>
              ...
              <p className="text-sm text-slate-300">Resumen de sugerencias recientes.</p>
            </div>
            <button
              type="button"
            ...
            <div className="flex flex-col gap-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
                <div className="rounded-lg border border-slate-800 bg-slate-900/80 p-3">
                  ...
                  <div className="text-lg font-semibold text-white">{usageSummary.summary.totalToday}</div>
                </div>
                <div className="rounded-lg border border-slate-800 bg-slate-900/80 p-3">
                  <div className="text-xs text-slate-400">Sugerencias últimos 7 días</div>
                  <div className="text-lg font-semibold text-white">{usageSummary.summary.totalLast7Days}</div>
                </div>
                <div className="rounded-lg border border-slate-800 bg-slate-900/80 p-3">
                  <div className="text-xs text-slate-400">Créditos usados hoy</div>
                  <div className="text-lg font-semibold text-white">{usageSummary.summary.creditsToday}</div>
                </div>
                <div className="rounded-lg border border-slate-800 bg-slate-900/80 p-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="rounded-lg border border-slate-800 bg-slate-900/80 p-3">
                  <div className="text-xs font-semibold text-white mb-2">Por tipo (hoy)</div>
                  <div className="flex flex-wrap gap-2 text-[11px] text-slate-200">
                    {usageSummary.summary.byActionTypeToday.length === 0 && <span className="text-slate-400">Sin datos</span>}
                    {usageSummary.summary.byActionTypeToday.map((item) => (
              ...
              <div className="rounded-lg border border-slate-800 bg-slate-900/80 p-3">
                <div className="text-xs font-semibold text-white mb-2">Últimos usos</div>
