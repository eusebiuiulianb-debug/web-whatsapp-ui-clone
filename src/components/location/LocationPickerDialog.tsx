...
type LocationConfirmPayload = LocationValue & {
  radiusKm: number;
};
...
type Props = {
  ...
  onPrimaryAction?: (next: LocationConfirmPayload) => void;
  onConfirm?: (next: LocationConfirmPayload) => void;
...
export function LocationPickerDialog({
  ...
  const primaryAction = onPrimaryAction ?? onConfirm;
  const selectionInvalid = Boolean(primaryAction) && queryDirty && geoQuery.trim().length > 0;
  const onPrimary = () => {
    if (selectionInvalid) {
